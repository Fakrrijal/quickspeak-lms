-- Harden teacher-fee historical scope.
-- Historical fee is based on recorded meetings + attendance.
-- It must not disappear because a teaching group is later deactivated,
-- reassigned, or its current group_type changes.
-- Absent remains non-payable (Rp0).

CREATE OR REPLACE FUNCTION public.get_teacher_fee_report(
    p_month integer,
    p_year integer,
...
RETURNS TABLE (
    teacher_id uuid, teacher_name text, teacher_code text,
    meeting_id uuid, session_date date, attendance_id uuid,
    attendance_recorded_at timestamptz, attendance_status text,
    teaching_group_id uuid, teaching_group_name text, level_id uuid, level_name text,
    package_type text, student_id uuid, student_name text, student_code text,
    fee_rate numeric, student_fee numeric, meeting_present_count bigint, meeting_fee numeric,
    is_fee_session_lead boolean, session_time timestamptz, session_number bigint,
    present_students bigint, fee numeric, status text, period_start date,
    earned_amount numeric, paid_amount numeric, outstanding_amount numeric, paid_at timestamptz,
    period_status text, period_earned numeric, period_paid numeric, period_outstanding numeric,
...
BEGIN
    ...
    attendance_details AS (
        SELECT
            ...
            CASE
                WHEN e.package_type = 'private' THEN m.id::text
                ELSE tg.id::text || ':' || m.session_date::text
            END AS fee_session_key
    ...
    logical_sessions_base AS (
        SELECT
            ...
            d.session_date,
            d.teaching_group_id,
            d.teaching_group_name,
            d.package_type,
            min(d.teacher_recorded_at) AS session_time,
            min(d.meeting_created_at) AS first_created_at,
            count(*)::bigint AS present_students,
            sum(d.fee_rate)::numeric AS fee,
        ...
        GROUP BY d.teacher_id, d.fee_session_key, d.session_date, d.teaching_group_id,
                 d.teaching_group_name, d.package_type
    ),
    logical_sessions AS (
        SELECT lsb.*,
               row_number() OVER (
                   PARTITION BY lsb.teacher_id, lsb.teaching_group_id
                   ORDER BY lsb.session_date, lsb.first_created_at
    ...
    report_periods AS (
        ...
        LEFT JOIN public.teacher_fee_periods AS tfp
          ON tfp.teacher_id = st.id
         AND tfp.period_start = v_period_start
    )
    SELECT
        rp.teacher_id, rp.teacher_name, rp.teacher_code,
        ...
        COALESCE(ls.present_students, 0),
        COALESCE(ls.fee, 0),
        COALESCE(d.attendance_id = ls.lead_attendance_id, false),
        ls.session_time, ls.session_number,
        ...
        rp.status, rp.earned_amount, rp.paid_amount,
        rp.earned_amount - rp.paid_amount,
    ...
    LEFT JOIN logical_sessions AS ls
      ON ls.teacher_id = d.teacher_id
     AND ls.fee_session_key = d.fee_session_key
    WHERE p_status = 'all' OR rp.status = p_status
    ORDER BY rp.teacher_name, rp.teacher_id,
             d.session_date DESC NULLS LAST,
             d.teacher_recorded_at DESC NULLS LAST,
             d.meeting_id DESC NULLS LAST;
END;
$$;

CREATE OR REPLACE FUNCTION public.admin_get_teacher_fee_periods(
    p_month integer,
    p_year integer
)
RETURNS TABLE (
    teacher_id uuid, teacher_name text, period_start date,
    earned_amount numeric, due_date date, status text, paid_at timestamptz
)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
...
BEGIN
    ...
    WITH valid_present_attendance AS (
        ...
        FROM public.meetings AS m
        JOIN public.attendance AS a
          ON a.meeting_id = m.id AND a.teacher_status = 'present'
        JOIN public.teaching_groups AS tg ON tg.id = m.teaching_group_id
        JOIN public.enrollments AS e ON e.id = m.enrollment_id
        WHERE m.session_date >= v_period_start
          AND m.session_date < v_period_end
    ...
    logical_sessions AS (
        SELECT v.teacher_id, v.teaching_group_id, v.session_date, v.package_type,
               CASE WHEN v.package_type = 'private' THEN count(*) * 15000
                    ELSE count(*) * 8000 END::numeric AS fee
        FROM valid_present_attendance AS v
        GROUP BY v.teacher_id, v.teaching_group_id, v.session_date, v.package_type,
                 CASE WHEN v.package_type = 'private' THEN v.id::text
                      ELSE v.teaching_group_id::text END
    ),
    live_amounts AS (
    ...
    SELECT
        t.id, p.full_name, v_period_start,
...
CREATE OR REPLACE FUNCTION public.admin_mark_teacher_fee_period_paid(
    p_teacher_id uuid,
    p_period_start date
)
