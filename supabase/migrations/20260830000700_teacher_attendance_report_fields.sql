-- Extend the existing authorized teacher attendance read model with fields
-- required by the report UI. Authorization remains restricted to own meetings.
...
BEGIN
    ...
    WHERE m.teacher_id = v_teacher_id AND m.session_date >= v_start_date AND m.session_date < v_end_date
