drop function if exists public.get_my_learning_state();

create function public.get_my_learning_state()
returns table (
  enrollment_id uuid,
  level_id uuid,
  level_name text,
...
begin
  ...
  group by e.id, e.level_id, l.name, l.level_number, e.package_type, e.status, e.session_limit, e.started_at, e.completed_at, tg.id, tg.name, t.id, tp.full_name, t.teacher_code
  order by e.created_at desc, e.id desc
  limit 1;
end;
$$;

grant execute on function public.get_my_learning_state() to authenticated;
