-- S3 Payment Proof Submission
-- Storage upload is client-side; this RPC atomically records the proof and
-- advances only the permitted payment and enrollment submission states.
...
DROP POLICY IF EXISTS "Students can insert own payment proofs" ON public.payment_proofs;
REVOKE INSERT ON public.payment_proofs FROM authenticated;

DROP POLICY IF EXISTS "Students can upload own payment proofs" ON storage.objects;
CREATE POLICY "Students can upload eligible payment proofs"
    ...
    WITH CHECK (
        ...
        AND EXISTS (
            ...
            WHERE students.profile_id = auth.uid()
              ...
              AND payments.status IN ('unpaid', 'rejected')
        )
    );

CREATE POLICY "Students can delete unreferenced payment proof uploads"
    ...
    USING (
        ...
        AND NOT EXISTS (
            ...
            WHERE payment_proofs.storage_path = name
        )
    );
...
REVOKE ALL ON FUNCTION public.submit_payment_proof(uuid, text, text, text, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.submit_payment_proof(uuid, text, text, text, integer)
TO authenticated;
