  ...
  return (
    <div className="space-y-6">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="border-b border-slate-200 px-5 py-5 sm:px-7"><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-blue-700">History</p><h2 className="mt-1 text-xl font-extrabold text-[#102449]">Payment History</h2><p className="mt-1 text-sm text-slate-600">Your recorded payment transactions.</p></div>
        <div className="px-3 py-2.5 sm:px-5"><table className="w-full table-fixed text-left text-sm"><colgroup><col className="w-[24%]" /><col className="w-[13%]" /><col className="w-[12%]" /><col className="w-[13%]" /><col className="w-[14%]" /><col className="w-[10%]" /><col className="w-[14%]" /></colgroup><thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-[0.1em] text-slate-500"><tr><th className="px-2.5 py-3">Invoice</th><th className="px-2.5 py-3">Payment Date</th><th className="px-2.5 py-3">Period</th><th className="px-2.5 py-3">Amount</th><th className="px-2.5 py-3">Payment Method</th><th className="px-2.5 py-3">Status</th><th className="px-2.5 py-3">Action</th></tr></thead>
          <tbody className="divide-y divide-slate-100">{pageRecords.map((item) => <tr key={item.id} className="align-top"><td className="px-2.5 py-3.5"><p className="break-all font-semibold leading-5 text-slate-900">{item.invoice_number}</p></td><td className="px-2.5 py-3.5 text-slate-700">{formatPaymentDate(item.created_at)}</td><td className="px-2.5 py-3.5 text-slate-700">{item.period}</td><td className="px-2.5 py-3.5 font-bold text-slate-900">Rp{item.amount.toLocaleString('id-ID')}</td><td className="px-2.5 py-3.5 text-slate-700">{formatMethod(item.payment_method)}</td><td className="px-2.5 py-3.5"><span className={`inline-flex rounded-full px-2 py-1 text-[11px] font-bold ${statusClass(item.status)}`}>{formatStatus(item.status)}</span></td><td className="px-2.5 py-3.5"><div className="flex flex-col items-start gap-1"><button type="button" onClick={() => setSelectedHistory(item)} className="font-bold text-blue-700 underline-offset-2 hover:underline">View</button><button type="button" onClick={() => downloadReceipt(item)} className="font-bold text-slate-700 underline-offset-2 hover:underline">Download</button></div></td></tr>)}{!pageRecords.length && <tr><td colSpan={7} className="px-2.5 py-8 text-center text-sm text-slate-500">No payment transactions recorded.</td></tr>}</tbody></table></div>
        <div className="flex flex-col gap-3 border-t border-slate-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs text-slate-500">Showing {showingStart}–{showingEnd} of {paymentHistory.length} transactions</p><div className="flex items-center gap-1.5">{paginationPages.map((page) => page === 'ellipsis-left' || page === 'ellipsis-right' ? <span key={page} className="px-2 text-xs text-slate-400">…</span> : <button key={page} type="button" onClick={() => setHistoryPage(Number(page))} className={`size-8 rounded-lg text-xs font-bold ${safePage === page ? 'bg-blue-600 text-white' : 'border border-slate-200 bg-white text-slate-700 hover:bg-slate-50'}`}>{page}</button>)}</div></div>
      </section>
