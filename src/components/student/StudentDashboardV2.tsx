  ...
  return (
    <div className="space-y-6 pb-2">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm" aria-labelledby="account-support-title">
        <div className="grid gap-5 px-6 py-5 sm:px-7 lg:grid-cols-2 xl:grid-cols-[minmax(0,1.2fr)_minmax(0,0.85fr)_minmax(0,1fr)] xl:items-center">
          <div className="flex items-start gap-3.5"><div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-emerald-50 text-emerald-700"><Icon name="check" /></div><div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-slate-500">Account Status</p><h2 id="account-support-title" className="mt-1 text-lg font-bold tracking-[-0.015em] text-[#102449]">Your account is active</h2><p className="mt-1 text-sm leading-6 text-slate-600">You can access your learning area and continue your current stage.</p></div></div>

          <div className="grid grid-cols-2 gap-4 border-t border-slate-200 pt-4 sm:border-l sm:border-t-0 sm:pl-5 sm:pt-0 xl:border-l xl:pl-5"><div><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-slate-500">Student</p><p className="mt-1.5 truncate text-sm font-bold text-slate-900">{profile.full_name}</p></div><div><p className="text-[10px] font-bold uppercase tracking-[0.13em] text-slate-500">Email</p><p className="mt-1.5 truncate text-sm font-bold text-slate-900">{profile.email}</p></div></div>

          <div className="border-t border-slate-200 pt-4 sm:col-span-2 xl:col-span-1 xl:border-l xl:border-t-0 xl:pl-5 xl:pt-0"><div className="flex items-center gap-2"><div className="flex size-8 items-center justify-center rounded-lg bg-blue-50 text-blue-700"><Icon name="help" /></div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-slate-500">Need help?</p></div><p className="mt-1.5 text-base font-bold text-[#102449]">Contact QuickSpeak Admin</p><p className="mt-1 text-sm leading-5 text-slate-600">Learning, payment, or account questions.</p><div className="mt-3 flex flex-wrap gap-2"><a href="mailto:quicspeaklms@gmail.com" className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 transition hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#102449]"><Icon name="mail" /> Email Admin</a><a href="https://wa.me/6282138138564?text=Halo%20QuickSpeak%20Admin%2C%20saya%20membutuhkan%20bantuan%20terkait%20student%20portal." target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg bg-[#102449] px-3 py-2 text-sm font-bold text-white transition hover:bg-[#17325f] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#102449]"><Icon name="whatsapp" /> WhatsApp Admin</a></div></div>
        </div>
      </section>
    </div>
  )
