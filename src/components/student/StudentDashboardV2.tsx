...
function Icon({ name }: { name: 'book' | 'calendar' | 'user' | 'arrow' | 'check' | 'help' | 'mail' | 'whatsapp' }) {
  ...
  return <svg aria-hidden="true" viewBox="0 0 24 24" className={common}><path d="M5 12h13M13 6l-6 6 6 6" /></svg>
}
    ...
    <div className="space-y-6 pb-2">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm" aria-labelledby="account-support-title">
        <div className="grid gap-0 px-6 py-5 sm:px-7 xl:grid-cols-[minmax(0,1.25fr)_minmax(0,0.85fr)_minmax(0,1fr)] xl:items-center">
          ...
          <div className="border-t border-slate-200 pt-4 xl:border-l xl:border-t-0 xl:pl-6 xl:pt-1"><div className="flex items-center gap-2"><div className="flex size-8 items-center justify-center rounded-lg bg-blue-50 text-blue-700"><Icon name="help" /></div><p className="text-[11px] font-bold uppercase tracking-[0.16em] text-slate-500">Need help?</p></div><p className="mt-1.5 text-base font-bold text-[#102449]">Contact QuickSpeak Admin</p><p className="mt-1 text-sm leading-5 text-slate-600">Learning, payment, or account questions.</p><div className="mt-3 grid grid-cols-2 gap-2"><a href="mailto:quicspeaklms@gmail.com" className="inline-flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-bold text-slate-700 transition hover:bg-slate-50 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#102449]"><Icon name="mail" /> Email Admin</a><a href="https://wa.me/6282138138564?text=Halo%20QuickSpeak%20Admin%2C%20saya%20membutuhkan%20bantuan%20terkait%20student%20portal." target="_blank" rel="noreferrer" className="inline-flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-lg bg-[#102449] px-3 py-2 text-sm font-bold text-white transition hover:bg-[#17325f] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#102449]"><Icon name="whatsapp" /> WhatsApp Admin</a></div></div>
        </div>
      </section>
    </div>
  )
