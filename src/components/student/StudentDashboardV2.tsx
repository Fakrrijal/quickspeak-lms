      ...
      <main className="grid gap-4 lg:grid-cols-[minmax(0,1.5fr)_minmax(320px,0.95fr)]">
              ...
              <div>
                <div className="flex flex-col gap-4 border-b border-slate-200 pb-5 sm:flex-row sm:items-end sm:justify-between">
                  <div>
                    ...
                    <h3 className="mt-1 text-[26px] font-extrabold leading-tight tracking-[-0.03em] text-[#102449]">{learningState.level_name}</h3>
                    <p className="mt-2 text-base font-bold text-blue-700">{formatPackage(learningState.package_type)}</p>
                  </div>
                  <span className={`inline-flex w-fit items-center rounded-md px-2.5 py-1.5 text-sm font-bold ${completed ? 'bg-emerald-50 text-emerald-700' : assigned ? 'bg-blue-50 text-blue-700' : 'bg-amber-50 text-amber-800'}`}>
                ...
                <div className="grid border-b border-slate-200 sm:grid-cols-2">
                  <div className="border-b border-slate-200 py-4 sm:border-b-0 sm:border-r sm:pr-6">
                    ...
                    <p className="mt-1.5 text-base font-bold text-slate-900">{learningState.teaching_group_name ?? 'Not assigned yet'}</p>
                  </div>
                  <div className="py-4 sm:pl-6">
                    <p className="text-[11px] font-bold uppercase tracking-[0.15em] text-slate-500">Teacher</p>
                    <p className="mt-1.5 text-base font-bold text-slate-900">{learningState.teacher_code ?? 'Not assigned yet'}</p>
                  </div>
                </div>
                ...
                {completed && (
                  <Link to="/student/learning" className="mt-4 inline-flex items-center gap-2 rounded-lg bg-[#102449] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#17325f] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#102449]">
                    Continue to Next Stage <Icon name="arrow" />
                  </Link>
                )}
              </div>
            ) : (
              <div className="border border-dashed border-slate-300 bg-slate-50 p-5 rounded-xl">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm" aria-labelledby="attendance-title">
          ...
          <div className="px-6 py-5 sm:px-7 sm:py-6">
            ...
            <dl className="grid grid-cols-3 divide-x divide-slate-200 pt-4">
              <div className="pr-3">
