...
function Icon({ name }: { name: 'book' | 'calendar' | 'user' | 'arrow' | 'check' | 'wallet' }) {
  ...
  return (
    <div className="space-y-6 pb-2">
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Student dashboard summary">
        <SummaryCard label="Current Learning" value={learningState?.level_name ?? '—'} detail={learningState ? `${learningState.teaching_group_name ?? 'Group not assigned'} · ${learningState.teacher_code ?? 'Teacher not assigned'} · ${formatPackage(learningState.package_type)}` : learningStateError ? 'Unable to load learning status' : 'No learning package yet'} to="/student/learning" icon="book" color="text-blue-700" iconBg="bg-blue-50" />
        <SummaryCard label="Attendance" value={attendanceLoading ? '…' : attendanceError ? '—%' : rate === null ? '—%' : `${rate.toFixed(0)}%`} detail={attendanceError ? 'Unable to load attendance details' : `${present} present · ${absent} absent · ${total} records`} to="/student/attendance" icon="calendar" color="text-emerald-700" iconBg="bg-emerald-50" />
        <SummaryCard label="Learning Progress" value={`${currentCompletedRows.length}/${currentLevelRows.length}`} detail={`${currentProgressPercent}% complete${nextChapter ? ` · Next: Chapter ${nextChapter.chapter_number}` : ''}`} to="/student/learning" icon="book" color="text-violet-700" iconBg="bg-violet-50" />
        <SummaryCard label="Payment" value="Open" detail="View your payment and enrollment details" to="/student-payment" icon="wallet" color="text-amber-800" iconBg="bg-amber-50" />
      </section>
        ...
        <div className="student-progress-summary px-6 py-5 sm:px-7 sm:py-6">
          {progressLoading ? (
            ...
            <div>
              ...
              <div className="student-next-step mt-5 flex flex-col gap-4 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                ...
                <Link to="/student/learning" className="inline-flex w-fit shrink-0 items-center gap-2 rounded-lg bg-[#102449] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#17325f]">{completed ? 'Continue to Next Stage' : 'Continue Learning'} <Icon name="arrow" /></Link>
              </div>
              ...
              <div className="mt-5 overflow-hidden rounded-xl border border-slate-200 bg-slate-50/50">
                ...
                {chaptersExpanded && <div className="border-t border-slate-200 bg-white p-3 sm:p-4"><div className="max-h-[250px] overflow-y-auto pr-1"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{currentLevelRows.map((row) => <div key={row.chapter_id} className={`rounded-xl border px-3.5 py-3 ${row.completed_at ? 'border-emerald-100 bg-emerald-50/50' : 'border-slate-200 bg-slate-50/60'}`}><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500">Chapter {row.chapter_number}</p><p className="mt-1 text-sm font-bold text-[#102449]">{row.chapter_title}</p><p className={`mt-1 text-xs font-bold ${row.completed_at ? 'text-emerald-700' : 'text-slate-500'}`}>{row.completed_at ? '✓ Completed' : 'Not completed yet'}</p></div>)}</div></div></div>}
              </div>
            </div>
          )}
        </div>
