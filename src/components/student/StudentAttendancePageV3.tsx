...
type IconName = 'calendar' | 'check' | 'x' | 'clock' | 'download' | 'arrow' | 'close'
...
function AttendanceDetail({ record, records, period, onClose }: { record: StudentAttendanceRecord, records: StudentAttendanceRecord[], period: string, onClose: () => void }) {
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="attendance-detail-title" className="mx-auto my-6 w-full max-w-4xl overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-2xl">
        <header className="flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5 sm:px-7">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">{record.student_code} · {period}</p>
          </div>
        ...
        <div className="space-y-5 p-6 sm:p-7">
          <section className="grid gap-3 sm:grid-cols-4">
            {[['Sessions', String(records.length)], ['Present', String(present)], ['Absent', String(absent)], ['Rate', rate === null ? '—' : `${rate.toFixed(1)}%`]].map(([label, value]) => (
          ...
          <section>
            <div className="mb-3"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-500">Meeting History</p><h3 className="mt-1 text-lg font-bold text-[#102449]">Meeting &amp; Attendance Detail</h3></div>
            <div className="space-y-3">
              {records.map((item, index) => (
                <article key={item.meeting_id} className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-bold text-slate-900">Meeting {index + 1} · {formatDate(item.session_date)}</p><p className="mt-1 flex items-center gap-1.5 text-xs text-slate-500"><Icon name="clock" /> Recorded {formatDateTime(item.teacher_recorded_at)}</p></div><StatusBadge status={item.teacher_status} /></div>
                  <div className="mt-4 grid gap-x-6 gap-y-3 text-sm sm:grid-cols-2">
                    ...
                    <div><p className="text-[10px] font-bold uppercase tracking-[0.12em] text-slate-500">Recorded Time</p><p className="mt-1 font-semibold text-slate-900">{formatDateTime(item.teacher_recorded_at)}</p></div>
                  </div>
                </article>
              ))}
            </div>
...
export function StudentAttendancePageV3() {
  ...
  const pageStart = attendance.length ? (safePage - 1) * PAGE_SIZE : 0
  ...
  const download = () => {
    ...
    downloadAdminAttendancePdf({
      ...
      filters: { student: `${attendance[0].student_name} (${attendance[0].student_code})`, teacher: 'All Teachers', teachingGroup: 'All Groups', period },
  ...
  return (
    <div className="space-y-5 pb-2">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-3 border-b border-slate-200 px-6 py-4.5 sm:px-7">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-500">Detailed History</p><h2 className="mt-0.5 text-xl font-extrabold tracking-[-0.02em] text-[#102449]">Attendance Records</h2><p className="mt-1 text-sm text-slate-500">Showing {period}.</p></div><button type="button" onClick={download} disabled={!attendance.length || attendanceLoading} className="inline-flex w-fit items-center justify-center gap-2 rounded-lg bg-[#102449] px-3.5 py-2 text-sm font-bold text-white transition hover:bg-[#17325f] disabled:cursor-not-allowed disabled:opacity-50"><Icon name="download" />Download PDF</button></div>
          <div className="border-t border-slate-200 bg-slate-50/70 px-0 pt-3 sm:pt-4"><div className="grid gap-2.5 sm:grid-cols-[minmax(0,220px)_minmax(0,160px)_1fr] sm:items-end"><label className="block text-sm font-semibold text-slate-700">Month<select value={month} onChange={(event) => setMonth(Number(event.target.value))} className="mt-1.5 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100">{Array.from({ length: 12 }, (_, index) => <option key={index + 1} value={index + 1}>{new Intl.DateTimeFormat('en', { month: 'long' }).format(new Date(2000, index))}</option>)}</select></label><label className="block text-sm font-semibold text-slate-700">Year<input type="number" value={year} onChange={(event) => setYear(Number(event.target.value))} className="mt-1.5 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100" /></label><div className="hidden sm:block"><p className="text-right text-xs text-slate-500">Attendance is recorded by your assigned teacher.</p></div></div></div>
        </div>

        <div className="border-t border-slate-200 px-6 py-3.5 sm:px-7">
          <div className="mb-3.5"><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-500">Attendance List</p><p className="mt-1 text-sm text-slate-500">Session records for {period}.</p></div>
