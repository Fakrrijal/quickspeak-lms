...
export function StudentLearningPageV2() {
  ...
  useEffect(() => {
    if (!(isAuthenticated && profile && !profileError && role === 'student' && status === 'active')) return
    let cancelled = false
    ...
    return () => { cancelled = true }
  }, [isAuthenticated, profile, profileError, role, status])
  ...
  const currentLevelRows = useMemo(
    () => state ? progressRows.filter((row) => row.level_number === state.level_number && row.chapter_id) : [],
    [progressRows, state],
  )
    ...
    <div className="space-y-6">
      ...
      <section className="border border-slate-200 bg-white shadow-sm" aria-labelledby="student-progress-title">
        ...
        <div className="p-6 sm:p-7">
          {progressLoading ? (
            <div className="space-y-3"><div className="h-5 w-48 animate-pulse rounded bg-slate-100" /><div className="h-2.5 w-full animate-pulse rounded bg-slate-100" /><div className="h-16 w-full animate-pulse rounded-xl bg-slate-50" /></div>
          ) : progressError ? (
            ...
            <details className="group rounded-xl border border-slate-200 bg-slate-50/50">
              <summary className="flex cursor-pointer list-none items-center gap-4 px-4 py-4 sm:px-5 [&::-webkit-details-marker]:hidden">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-extrabold text-[#102449]">{currentCompletedCount} / {currentLevelRows.length} chapters completed</p>
                  <div className="mt-2 h-2.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-blue-600 transition-all" style={{ width: `${currentProgressPercent}%` }} /></div>
                </div>
                <span className="shrink-0 text-sm font-extrabold text-blue-700">{currentProgressPercent}%</span>
              ...
              <div className="max-h-[360px] overflow-y-auto border-t border-slate-200 px-4 py-4 sm:px-5">
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  {currentLevelRows.map((row) => (
                    <article key={row.chapter_id} className={`rounded-xl border px-3.5 py-3 ${row.completed_at ? 'border-emerald-100 bg-emerald-50/60' : 'border-slate-200 bg-white'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4">
          <div className="w-full max-w-md border border-slate-200 bg-white p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.15em] text-blue-700">Continue Learning</p>
