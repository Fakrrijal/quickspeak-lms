...
function StatusBadge({ status }: { status: StudentAttendanceRecord['teacher_status'] }) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-bold ${present ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
...
function AttendanceDetail({ record, records, periodLabel, onClose }: { record: StudentAttendanceRecord, records: StudentAttendanceRecord[], periodLabel: string, onClose: () => void }) {
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/40 p-4">
      <div role="dialog" aria-modal="true" aria-labelledby="attendance-detail-title" className="mx-auto my-6 w-full max-w-4xl overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-2xl">
        <header className="flex items-start justify-between gap-4 border-b border-slate-200 bg-white px-6 py-5 sm:px-7">
          ...
          <button type="button" onClick={onClose} aria-label="Close attendance detail" className="inline-flex size-9 items-center justify-center rounded-lg border border-slate-200 text-slate-600 transition hover:bg-slate-50 hover:text-slate-900"><Icon name="close" /></button>
        </header>
        ...
        <div className="space-y-5 p-6 sm:p-7">
          ...
          <section>
            <div className="mb-3 flex items-center justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-500">Meeting History</p><h3 className="mt-1 text-lg font-bold text-[#102449]">Meeting &amp; Attendance Detail</h3></div></div>
            <div className="space-y-3">
              {records.map((item, index) => (
                <article key={item.meeting_id} className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-bold text-slate-900">Meeting {index + 1} · {date(item.session_date)}</p><p className="mt-1 flex items-center gap-1.5 text-xs text-slate-500"><Icon name="clock" /> Recorded {dateTime(item.teacher_recorded_at)}</p></div><StatusBadge status={item.teacher_status} /></div>
...
function StudentAttendancePage() {
  const { isAuthenticated, loading, profile, profileError, profileLoading, role, status } = useAuthContext()
  ...
  const download = () => {
    ...
    downloadAdminAttendancePdf({
      records: attendance.map((item) => ({ ...item, teacher_id: '', student_id: '', student_name: item.student_name, package_type: item.package_type, teaching_group_id: item.teaching_group_id })),
      generatedBy: profile.full_name || 'Student',
      generatedAt: dateTime(new Date().toISOString()),
      filters: { student: `${attendance[0].student_name} (${attendance[0].student_code})`, teacher: 'All Teachers', teachingGroup: 'All Groups', period: periodLabel },
      filename: `student-attendance-${attendance[0].student_code}-${year}-${String(month).padStart(2, '0')}.pdf`,
    })
  }

  if (loading || profileLoading) return <section className="border border-slate-200 bg-white p-6 shadow-sm"><div className="h-2.5 w-28 animate-pulse rounded-full bg-slate-200" /><div className="mt-4 h-8 w-72 max-w-full animate-pulse rounded-lg bg-slate-100" /><div className="mt-3 h-4 w-96 max-w-full animate-pulse rounded-full bg-slate-100" /></section>
  ...
  return (
    <div className="space-y-6 pb-2">
      <header className="flex flex-col gap-4 border-b border-slate-200 pb-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <Link to="/student" className="inline-flex w-fit items-center gap-2 text-sm font-bold text-blue-700 hover:text-blue-800">Back to Dashboard <Icon name="arrow" /></Link>
      </header>

      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[{ label: 'Total Sessions', value: summary.total, icon: 'calendar' as const, tone: 'bg-blue-50 text-blue-700' },{ label: 'Present', value: summary.present, icon: 'check' as const, tone: 'bg-emerald-50 text-emerald-700' },{ label: 'Absent', value: summary.absent, icon: 'x' as const, tone: 'bg-rose-50 text-rose-700' },{ label: 'Attendance Rate', value: summary.rate === null ? '—' : `${summary.rate.toFixed(1)}%`, icon: 'clock' as const, tone: 'bg-amber-50 text-amber-700' }].map((item) => (
          <article key={item.label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-500">{item.label}</p><p className="mt-2 text-2xl font-extrabold tracking-[-0.04em] text-[#102449]">{item.value}</p></div><div className={`flex size-10 items-center justify-center rounded-lg ${item.tone}`}><Icon name={item.icon} /></div></div></article>
        ))}
      </section>

      <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 px-6 py-5 sm:px-7">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.15em] text-slate-500">Detailed History</p><h2 className="mt-1 text-xl font-extrabold tracking-[-0.02em] text-[#102449]">Attendance Records</h2><p className="mt-1 text-sm text-slate-500">Showing {periodLabel}.</p></div><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={download} disabled={!attendance.length || attendanceLoading} className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#102449] px-4 py-2.5 text-sm font-bold text-white transition hover:bg-[#17325f] disabled:cursor-not-allowed disabled:opacity-50"><Icon name="download" />Download PDF</button></div></div>
        ...
        {recordsExpanded && <div id="attendance-records-content" className="border-t border-slate-200">
          ...
          <div className="p-6 sm:p-7">
            {attendanceLoading ? <div className="space-y-3">{[1, 2, 3, 4].map((item) => <div key={item} className="h-16 animate-pulse rounded-xl bg-slate-100" />)}</div> : attendance.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-6 py-10 text-center"><div className="mx-auto flex size-11 items-center justify-center rounded-xl bg-white text-slate-500 shadow-sm"><Icon name="calendar" /></div><h3 className="mt-4 text-base font-bold text-slate-900">No attendance records</h3><p className="mx-auto mt-1 max-w-md text-sm leading-6 text-slate-500">There are no attendance records for {periodLabel}.</p></div> : (
              <div className="h-[600px] overflow-y-scroll overflow-x-auto rounded-xl border border-slate-200">
                <table className="min-w-[1040px] w-full text-left text-sm">
                  <thead className="sticky top-0 z-10 bg-slate-50 text-[10px] uppercase tracking-[0.12em] text-slate-500 shadow-sm"><tr><th className="px-4 py-3.5 font-bold">Date</th><th className="px-4 py-3.5 font-bold">Teaching Group</th><th className="px-4 py-3.5 font-bold">Teacher</th><th className="px-4 py-3.5 font-bold">Level</th><th className="px-4 py-3.5 font-bold">Package</th><th className="px-4 py-3.5 font-bold">Status</th><th className="px-4 py-3.5 font-bold">Recorded Time</th><th className="px-4 py-3.5 text-right font-bold">Action</th></tr></thead>
                  <tbody>{attendance.map((item) => <tr key={item.meeting_id} className="border-t border-slate-200 bg-white transition hover:bg-slate-50"><td className="px-4 py-4 font-semibold text-slate-900">{date(item.session_date)}</td><td className="px-4 py-4 text-slate-700">{item.teaching_group_name}</td><td className="px-4 py-4"><p className="font-semibold text-slate-900">{item.teacher_name}</p><p className="mt-0.5 text-xs text-slate-500">{item.teacher_code}</p></td><td className="px-4 py-4 text-slate-700">{item.level_name}</td><td className="px-4 py-4 text-slate-700">{packageLabel(item.package_type)}</td><td className="px-4 py-4"><StatusBadge status={item.teacher_status} /></td><td className="px-4 py-4 whitespace-nowrap text-slate-600">{dateTime(item.teacher_recorded_at)}</td><td className="px-4 py-4 text-right"><button type="button" onClick={() => setDetail(item)} className="inline-flex items-center gap-1.5 font-bold text-blue-700 hover:text-blue-800">View Detail <Icon name="arrow" /></button></td></tr>)}</tbody>
                </table>
              </div>
            )}
          </div>
