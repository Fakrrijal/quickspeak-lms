...
function getUnpaidAttendanceCount(report: AdminTeacherFeeReport) {
  if (report.status === 'paid') return 0
  return report.detail_entries.filter((entry) => entry.attendance_status === 'present').length
}

function formatAttendanceLabel(count: number) {
  return `${count} attendance${count === 1 ? '' : 's'} unpaid`
}
...
function TeacherFeeDetailModal({ report, month, year, onClose }: { report: AdminTeacherFeeReport, month: number, year: number, onClose: () => void }) {
  ...
  const unpaidAttendanceCount = getUnpaidAttendanceCount(report)
      ...
      <div className="space-y-5 p-6">
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            ...
            <p className="mt-1 text-xs text-slate-500">{formatAttendanceLabel(unpaidAttendanceCount)}</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-4">
        ...
        <section className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-5">
            ...
            <div><dt className="text-slate-500">Outstanding</dt><dd className="mt-1 font-medium text-slate-900">{formatAmount(report.outstanding_amount)}</dd><p className="mt-1 text-xs text-slate-500">{formatAttendanceLabel(unpaidAttendanceCount)}</p></div>
...
function AdminTeacherFeesPage() {
  ...
  useEffect(() => { setCurrentPage(1) }, [month, search, statusFilter, teacherFilter, year])
  const totalPages = Math.max(1, Math.ceil(matchingReports.length / pageSize))
  useEffect(() => { if (currentPage > totalPages) setCurrentPage(totalPages) }, [currentPage, totalPages])
  const paginatedReports = useMemo(() => matchingReports.slice((currentPage - 1) * pageSize, currentPage * pageSize), [currentPage, matchingReports])
  ...
  const showingStart = matchingReports.length === 0 ? 0 : ((currentPage - 1) * pageSize) + 1
  const showingEnd = Math.min(currentPage * pageSize, matchingReports.length)
