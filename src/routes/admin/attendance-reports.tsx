...
function AdminAttendanceReportsPage() {
  ...
  const totalPages = Math.max(1, Math.ceil(studentSummary.length / pageSize))
  useEffect(() => { if (currentPage > totalPages) setCurrentPage(totalPages) }, [currentPage, totalPages])
  const paginatedStudentSummary = useMemo(() => studentSummary.slice((currentPage - 1) * pageSize, currentPage * pageSize), [currentPage, studentSummary])
  const showingStart = studentSummary.length === 0 ? 0 : ((currentPage - 1) * pageSize) + 1
  const showingEnd = Math.min(currentPage * pageSize, studentSummary.length)
