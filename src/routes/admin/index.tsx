...
import {
  ...
  getEditablePaidEnrollmentsForStudent,
...
function formatClassType(classType: 'private' | 'semi_private') {
  return classType === 'private' ? 'Private' : 'Semi-private'
}
...
function AdminStudentManagementPage() {
  ...
  const loadLevels = useCallback(async () => {
    try {
      ...
      // Editing will surface the actionable error if the list cannot be loaded.
    }
  }, [])
  ...
  const openStudentEditor = async (student: AdminStudentDirectoryItem) => {
    ...
    setEditPackageType(student.class_types[0] ?? 'private')
    setEditError(null)
    setPaidEnrollments([])
    ...
    try {
      ...
      if (enrollments.length === 1) {
        setEditLevelId(enrollments[0].level_id)
        setEditPackageType(enrollments[0].package_type)
      } else if (enrollments.length > 1) {
        setEditError('This student has more than one payment-approved enrollment. Resolve the enrollment history before editing this student.')
      }
    } catch (error) {
  ...
  const closeStudentEditor = () => {
    if (isSaving || isRemovingStudent) return
    setEditingStudent(null)
    setEditLevelId('')
    setEditPackageType('private')
    setEditError(null)
    setPaidEnrollments([])
  ...
  const saveStudent = async () => {
    ...
    try {
      if (correctionEnrollment) {
        const changed = editLevelId !== correctionEnrollment.level_id
          || editPackageType !== correctionEnrollment.package_type

        if (changed) {
          await adminEditPaidEnrollment(
            correctionEnrollment.id,
            editLevelId,
            editPackageType,
          )
        }
      } else if (editLevelId !== editingStudent.level?.id) {
        // Students without a payment-approved enrollment keep the existing base-level workflow.
        const { supabase } = await import('../../lib/supabase')
        const { error } = await supabase.rpc('admin_update_student_level', {
          p_student_id: editingStudent.id,
          p_level_id: editLevelId,
        })
      ...
      setEditError(error instanceof Error ? error.message : 'Unable to save the student correction.')
    } finally {
      setIsSaving(false)
    }
  }
            ...
            <table className="min-w-full divide-y divide-slate-200 text-left text-sm">
              <thead className="bg-slate-50 text-slate-600">
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold">Class Type</th>
              ...
              <tbody className="divide-y divide-slate-200">
                {filteredStudents.map((student) => (
                  <tr key={student.id} className="text-slate-700">
                    <td className="px-4 py-3 font-medium text-slate-900">{student.profile?.full_name ?? 'Name unavailable'}</td>
                    <td className="px-4 py-3">{student.profile?.email ?? '—'}</td>
                    <td className="px-4 py-3">{student.profile?.phone ?? '—'}</td>
                    <td className="px-4 py-3">{student.student_code}</td>
                    <td className="px-4 py-3">{student.profile?.created_at ? new Intl.DateTimeFormat('en-US').format(new Date(student.profile.created_at)) : '—'}</td>
                    <td className="px-4 py-3">{student.level?.name ?? 'Level unavailable'}</td>
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {student.class_types.length > 0
                          ? student.class_types.map((classType) => (
                              <span key={classType} className="rounded-full bg-indigo-50 px-2.5 py-1 text-xs font-medium text-indigo-700">
                    ...
                    <td className="px-4 py-3">{student.teaching_group_names.length > 0 ? student.teaching_group_names.join(', ') : 'Not Assigned'}</td>
                    <td className="px-4 py-3"><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs font-medium text-slate-700">{formatStatus(student.profile?.status ?? null)}</span></td>
                    <td className="px-4 py-3"><button type="button" onClick={() => void openStudentEditor(student)} className="font-medium text-slate-900 underline">Edit</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4" role="presentation">
          <section role="dialog" aria-modal="true" aria-labelledby="edit-student-title" className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <dl className="mt-5 space-y-3 text-sm">
              <div><dt className="font-medium text-slate-700">Student</dt><dd>{editingStudent.profile?.full_name ?? 'Name unavailable'}</dd></div>
              <div><dt className="font-medium text-slate-700">Student Code</dt><dd>{editingStudent.student_code}</dd></div>
              <div><dt className="font-medium text-slate-700">Student Base Level</dt><dd>{editingStudent.level?.name ?? 'Level unavailable'}</dd></div>
              {correctionEnrollment && (
                <>
                  <div><dt className="font-medium text-slate-700">Existing Operational Enrollment</dt><dd>{correctionEnrollment.levels?.name ?? 'Level unavailable'} · {formatClassType(correctionEnrollment.package_type)}</dd></div>
                  <div><dt className="font-medium text-slate-700">Payment</dt><dd>Approved · Rp{correctionEnrollment.price.toLocaleString('id-ID')}</dd></div>
                  <div><dt className="font-medium text-slate-700">Teaching Group</dt><dd>{editingStudent.teaching_group_names.length > 0 ? editingStudent.teaching_group_names.join(', ') : 'Not Assigned'}</dd></div>
                </>
              )}
            </dl>

            {isLoadingEnrollments ? (
              <p className="mt-5 text-sm text-slate-600">Loading enrollment state...</p>
            ) : hasMultiplePaidEnrollments ? (
              <p className="mt-5 rounded-lg bg-red-50 p-3 text-sm text-red-700">{editError}</p>
            ) : (
              <div className="mt-5 space-y-4">
                <label className="block text-sm font-medium text-slate-700">
                  {isCorrectionMode ? 'Level' : 'Level'}
                  <select value={editLevelId} onChange={(event) => setEditLevelId(event.target.value)} disabled={isSaving || isRemovingStudent} className="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 disabled:bg-slate-100">
                ...
                {isCorrectionMode && (
                  <label className="block text-sm font-medium text-slate-700">
                    Class Type
                    <select value={editPackageType} onChange={(event) => setEditPackageType(event.target.value as 'private' | 'semi_private')} disabled={isSaving || isRemovingStudent} className="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 disabled:bg-slate-100">
                      <option value="private">Private</option>
                      <option value="semi_private">Semi-private</option>
                    </select>
                  </label>
                ...
                <p className="rounded-lg bg-slate-50 p-3 text-sm text-slate-700">
                  {isCorrectionMode
                    ? `Save will correct the operational enrollment to ${levels.find((level) => level.id === editLevelId)?.name ?? 'the selected level'} · ${formatClassType(editPackageType)}. Existing invoice and payment history are preserved.`
                    : 'This changes the level used for future enrollments only. Existing enrollment history is unchanged.'}
                </p>
              </div>
            )}

            {editError && !hasMultiplePaidEnrollments && <p className="mt-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">{editError}</p>}

            {editingStudent.is_active && (
              <button type="button" onClick={() => setIsRemoveConfirmationOpen(true)} disabled={isSaving || isRemovingStudent} className="mt-6 text-sm font-medium text-red-700 underline disabled:opacity-50">Remove Student</button>
            )}

            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={closeStudentEditor} disabled={isSaving || isRemovingStudent} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700">Cancel</button>
              <button type="button" onClick={() => void saveStudent()} disabled={!editLevelId || isSaving || isRemovingStudent || isLoadingEnrollments || hasMultiplePaidEnrollments} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">
                {isSaving ? 'Saving...' : isCorrectionMode ? 'Save Correction' : 'Save'}
              </button>
            </div>
          </section>
        </div>
      ...
      {editingStudent && isRemoveConfirmationOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/40 p-4" role="presentation">
          <section role="dialog" aria-modal="true" aria-labelledby="remove-student-title" className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-6 flex justify-end gap-3">
              <button type="button" onClick={() => setIsRemoveConfirmationOpen(false)} disabled={isRemovingStudent} className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700">Cancel</button>
              <button type="button" onClick={() => void removeStudent()} disabled={isRemovingStudent} className="rounded-lg bg-red-700 px-4 py-2 text-sm font-medium text-white disabled:opacity-50">{isRemovingStudent ? 'Removing...' : 'Remove Student'}</button>
            </div>
          </section>
        </div>
      )}
