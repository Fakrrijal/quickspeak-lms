...
function ForgotPasswordPage() {
  ...
  return (
    <div className="auth-recovery-page mx-auto max-w-md py-2">
      <section className="rounded-2xl border border-slate-200 bg-white p-7 shadow-[0_18px_50px_rgba(15,35,75,0.08)] sm:p-9">
        <div className="text-center">
          ...
          <div className="mt-2 text-sm leading-6 text-slate-600">Enter the email address associated with your QuickSpeak account.</div>
        </div>

        {sent ? (
          <div className="mt-7 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm leading-6 text-emerald-800">
            <div className="font-semibold text-emerald-900">Reset link sent</div>
            <div className="mt-1">If an account exists for this email, a password reset link has been sent. Please check your inbox.</div>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            <div>
              ...
              <input
                ...
                required
                autoComplete="email"
                className="auth-recovery-input h-12 w-full rounded-xl border border-slate-200 bg-white px-4 text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#1b5dd7] focus:ring-4 focus:ring-blue-100"
              />
            </div>
