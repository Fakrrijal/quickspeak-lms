...
import { getMyTeacherAttendance, getMyTeacherAttendanceGroups } from '../services/teacher-attendance.service'
...
function DatePicker({ label, value, onChange, min }: { label: string, value: string, onChange: (value: string) => void, min?: string }) {
  return (
    <label className="block min-w-[170px] text-xs font-bold uppercase tracking-[0.14em] text-slate-500">
      ...
      <span className="relative mt-1.5 block">
        <input
          ...
          className="h-11 w-full rounded-xl border border-slate-300 bg-white px-3.5 text-sm font-semibold normal-case tracking-normal text-slate-800 shadow-sm outline-none transition focus:border-blue-500 focus:ring-2 focus:ring-blue-100"
        />
      </span>
    </label>
  )
...
export function TeacherDashboardV2() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then((results) => {
        if (cancelled) return
        const groupData = results[0] as Awaited<ReturnType<typeof getMyTeacherAttendanceGroups>>
        const meetingData = results.slice(1, 1 + months.length).flat() as TeacherAttendanceMeeting[]
        const feeData = results.slice(1 + months.length) as MyTeacherFeeReport[]
        setGroups(groupData)
        setMeetings(meetingData)
        setFeeReports(feeData)
        setLoading(false)
      })
      .catch((loadError) => {
        if (cancelled) return
      ...
      {loading ? (
        ...
        <>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[
              { label: 'Classes', value: classes, icon: 'group' as const, tone: 'bg-blue-50 text-blue-700' },
              { label: 'Active Students', value: activeStudents, icon: 'users' as const, tone: 'bg-emerald-50 text-emerald-700' },
              { label: 'Fee', value: formatRupiah(earnedFee), icon: 'wallet' as const, tone: 'bg-amber-50 text-amber-800' },
              { label: 'Attendance', value: attendanceCount, icon: 'calendar' as const, tone: 'bg-violet-50 text-violet-700' },
            ].map((card) => (
              <article key={card.label} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex size-10 items-center justify-center rounded-lg bg-slate-50"><span className={card.tone.split(' ')[1]}><Icon name={card.icon} /></span></div>
