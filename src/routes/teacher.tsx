        ...
        <div className="space-y-6 p-6">
              ...
              {records.map((record, index) => (
                <article key={record.meeting_id} className="rounded-xl border border-slate-200 p-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    ...
                    <span className={`inline-flex w-fit rounded-full px-2.5 py-1 text-xs font-bold ${record.teacher_status === 'present' ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
                      {record.teacher_status === 'present' ? 'Present' : 'Absent'}
                    </span>
                  </div>
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <dl className="mt-4 grid gap-4 sm:grid-cols-4">
              <div><dt className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Total Attendance</dt><dd className="mt-1 text-xl font-extrabold text-[#102449]">{overall.total_records}</dd></div>
                  ...
                  {historyLoading ? (
                    ...
                    studentSummary.map((student) => (
                      <tr key={`${student.student_id}:${student.teaching_group_id}`} className="hover:bg-slate-50">
                        ...
                        <td className="px-5 py-3 text-right font-semibold">{student.total_sessions}</td>
