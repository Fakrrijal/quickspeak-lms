...
export const Route = createFileRoute('/teacher/change-password')({ component: TeacherChangePasswordPage })

function TeacherChangePasswordPage() {
  const { isAuthenticated, loading: authLoading, profileLoading, profileError, role, status, user, profile } = useAuthContext()
  const navigate = useNavigate()
  const canRender = !authLoading && !profileLoading && isAuthenticated && !profileError && role === 'teacher' && status === 'active'
