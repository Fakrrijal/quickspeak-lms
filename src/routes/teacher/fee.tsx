...
function TeacherFeeDetailModal({ detailEntries, studentSummaries, dateRangeLabel, detailReconcilesPeriod, onClose }: { detailEntries: TeacherFeeDetailEntry[], studentSummaries: TeacherFeeStudentSummary[], dateRangeLabel: string, detailReconcilesPeriod: boolean, onClose: () => void }) {
  ...
  const paginatedMeetings = useMemo(() => meetings.slice((meetingPage - 1) * PAGE_SIZE, meetingPage * PAGE_SIZE), [meetings, meetingPage])
  ...
  useEffect(() => {
    setMeetingPage(1)
    setSummaryPage(1)
  }, [detailEntries, studentSummaries])

  useEffect(() => {
    const totalPages = Math.max(1, Math.ceil(meetings.length / PAGE_SIZE))
    if (meetingPage > totalPages) setMeetingPage(totalPages)
  }, [meetings.length, meetingPage])

  useEffect(() => {
    const totalPages = Math.max(1, Math.ceil(studentSummaries.length / PAGE_SIZE))
    if (summaryPage > totalPages) setSummaryPage(totalPages)
...
function TeacherFeePage() {
  ...
  return (
    <div className="qs-fee-report-frame mx-auto w-full space-y-6">
      ...
      {showDetailModal && <TeacherFeeDetailModal detailEntries={visibleDetailEntries} studentSummaries={studentSummaries} dateRangeLabel={dateRangeLabel} detailReconcilesPeriod={detailReconcilesPeriod} onClose={() => setShowDetailModal(false)} />}
    </div>
  )
}
