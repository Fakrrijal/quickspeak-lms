...
function TeacherFeeDetailModal({ detailEntries, studentSummaries, month, year, detailReconcilesPeriod, onClose }: { detailEntries: TeacherFeeDetailEntry[], studentSummaries: TeacherFeeStudentSummary[], month: number, year: number, detailReconcilesPeriod: boolean, onClose: () => void }) {
  ...
  const reconciliationMessage = detailReconcilesPeriod ? 'Detail reconciles with Teacher Fee total.' : 'Detail does not currently reconcile with the Teacher Fee total.'
...
function TeacherFeePage() {
  ...
  return (
    <div className="qs-fee-report-frame mx-auto w-full space-y-6">
      <header className="border-b border-slate-200 pb-5"><p className="text-[11px] font-bold uppercase tracking-[0.2em] text-blue-700">Teacher Portal</p><div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-3xl font-extrabold tracking-[-0.03em] text-[#102449]">Teacher Fee</h1><p className="mt-1.5 text-sm leading-6 text-slate-600">Review your current fee statement and attendance-based earnings.</p></div><span className={`inline-flex w-fit items-center rounded-full border px-3 py-1.5 text-xs font-bold uppercase tracking-[0.12em] ${statusTone}`}>{summary.status}</span></div></header>
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">Statement Period</p><p className="mt-1 text-lg font-bold text-[#102449]">{formatMonth(year, month)}</p></div><div className="flex flex-wrap gap-3"><label className="text-sm font-semibold text-slate-700">Month<select value={month} onChange={(event) => setMonth(Number(event.target.value))} className="ml-2 rounded-lg border border-slate-300 bg-white px-3 py-2 font-medium">{Array.from({ length: 12 }, (_, index) => <option key={index + 1} value={index + 1}>{new Intl.DateTimeFormat('en', { month: 'long' }).format(new Date(2000, index))}</option>)}</select></label><label className="text-sm font-semibold text-slate-700">Year<input type="number" min="2000" max="9999" value={year} onChange={(event) => setYear(Number(event.target.value))} className="ml-2 w-24 rounded-lg border border-slate-300 bg-white px-3 py-2 font-medium" /></label><div className="flex items-end gap-2">{(['all', 'paid', 'unpaid'] as const).map((filter) => <button key={filter} type="button" onClick={() => setStatusFilter(filter)} className={statusFilter === filter ? 'rounded-lg bg-[#102449] px-3 py-2 text-sm font-bold text-white' : 'rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50'}>{filter === 'all' ? 'All' : filter === 'paid' ? 'Paid' : 'Unpaid'}</button>)}</div><button type="button" onClick={() => setShowDetailModal(true)} disabled={loading || visibleDetailEntries.length === 0} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50 disabled:opacity-50">View Detail</button><button type="button" onClick={() => void handleDownloadPdf()} disabled={isExporting || loading} className="rounded-lg bg-[#102449] px-3 py-2 text-sm font-bold text-white hover:bg-[#17325f] disabled:opacity-50">{isExporting ? 'Preparing PDF...' : 'Download PDF'}</button></div></div></section>
      <section className="grid gap-4 md:grid-cols-3">{[['Earned', summary.earned, 'bg-blue-50 text-blue-700'], ['Paid', summary.paid, 'bg-emerald-50 text-emerald-700'], ['Outstanding', summary.outstanding, 'bg-amber-50 text-amber-800']].map(([label, amount, tone]) => <article key={String(label)} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><p className="text-sm font-semibold text-slate-600">{label}</p><span className={`rounded-lg px-2 py-1 text-[10px] font-bold uppercase tracking-[0.12em] ${tone}`}>This Period</span></div><p className="mt-3 text-2xl font-extrabold tracking-[-0.03em] text-[#102449]">{formatAmount(Number(amount))}</p></article>)}</section>
